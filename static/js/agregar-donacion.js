...
const comunasupdate = () => {
    ...
    let indice = reg.selectedIndex;
    for (x in regcom[indice-1].comunas){
        let c = document.createElement("option");
        c.value =regcom[indice-1].comunas[x].id
        c.text =regcom[indice-1].comunas[x].nombre
        com.appendChild(c);
    }
};

reg.addEventListener("change",comunasupdate);
...
const validateRegion = (region) => {
    if (region.value=="none") return false;

    for (x in regcom) {
        if (region.value == regcom[x].id) {
            return true;
        }
    }
    return false;
...
const validateComuna = (comuna) => {
    ...
    for (x in regcom[indice-1].comunas) {
        if (comuna.value == regcom[indice-1].comunas[x].id) {
            return true;
        }
    }
    return false;
...
const validateCalle = (calle) => {
    ...
    let re = /^[a-zA-Z\s]+[\d]+$/;
    let format = re.test(calle);

    return format;
}

const validateTipo = (tipo) => {
    if (tipo.value=="none") return false;
    return true;
}

const validateCantidad = (cantidad) => {
    if (!cantidad) return false;
    if (parseInt(cantidad) <= 0) return false

    let re = /^[A-Za-z0-9\s]+$/;
    let formatV = re.test(cantidad);
...
const validateFecha = (fecha) => {
    ...
    let lengthV = (fecha.length <= 10);

    let re =  /^\d{4}\-(0[1-9]|1[012])\-(0[1-9]|[12][0-9]|3[01])$/;
    let formatV = re.test(fecha);
    ...
    if (parseInt(f[0]) < year) {
        return false;
    } else if (parseInt(f[0])== year) {
...
const validateFiles = (file) => {
    ...
    for (const f of file) {
        ...
        let fileFamily = f.type.split("/")[0];
        typeValid &&= fileFamily == "image" || f.type == "application/pdf";
      }
...
const validateNombre = (nombre) => {
    ...
    let lengthV = 3 <= nombre.length && nombre.length <= 80;
    let re = /[a-zA-ZáéíóúÁÉÍÓÚüÜñÑ]+(?:\s+[a-zA-ZáéíóúÁÉÍÓÚüÜñÑ]+){0,5}(?<!\s)$/;
    let formatV = re.test(nombre);
...
const validateForm = () => {
    ...
    let regionn = region.options[region.selectedIndex];
    let comuna = document.getElementById("comuna");
    let comunaa = comuna.options[comuna.selectedIndex];
    let calle = myForm["calle-numero"].value;
    let tipo = document.getElementById("tipo")
    let tipoo = tipo.options[tipo.selectedIndex];
    let cantidad = myForm["cantidad"].value;
    let fecha = myForm["fecha-disponibilidad"].value;
    let foto1 = document.getElementById("foto-1").files;
    let foto2 = document.getElementById("foto-2").files;
    let foto3 = document.getElementById("foto-3").files;
    ...
    if (!validateRegion(regionn)) {
        setInvalidInput("Región");
    }
    if (!validateComuna(comunaa)) {
        setInvalidInput("Comuna");
    }
    if (!validateCalle(calle)) {
        setInvalidInput("Calle y número");
    }
    if (!validateTipo(tipoo)) {
    ...
    if (foto2.length != 0) {
        if (!validateFiles(foto2)) {
            setInvalidInput("Foto 2");
        }
    }
    if (foto3.length != 0) {
        if (!validateFiles(foto3)) {
    ...
    if (!validateCelular(celular)) {
        setInvalidInput("Número celular contacto");
    }
    ...
    if (!isValid) {
        ...
        let conf = document.getElementById("confirmacion");
        conf.hidden=false
